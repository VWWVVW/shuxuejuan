...
#context sxj-title(size: env-get("font-size").huge)
...
#ans(ans-type: SXJ-BODY-TYPE.ANS.PF, composer: COMPOSER.GRID)[
  ...
  #qst(level: 3)[
    ...
    故$gcd(p_1+p_2, q_1+q_2)=1$，$c$的既约分数表达即为$(p_1+p_2)/(q_1+q_2)$，且$a,c$满足 #rfq(<qst-ford-2-circle>) 中条件。\
...
#qst[
  对 #rfq(<qst-ford>) 的补充：自备纸张，证明对$a,b in NN^+$，$gcd(a, b)=1<=>exists u,v in ZZ, u a + v b = 1$。
]
